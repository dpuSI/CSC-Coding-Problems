 ...
 *  of directed edges that emanate from that vertex. No vertex is reachable
 *  from a vertex of outdegree 0, which is called a sink; a vertex of indegree
 *  0, which is called a source, is not reachable from any other vertex. A 
 *  digraph where self-loops are allowed and every vertex has outdegree 1 is
 *  called a map (a function from the set of integers from 0 to V–1 onto
 *  itself).
 *  Write a program Degrees.java that implements the following API: 
 *  Degrees(Digraph G)
 *  int indegree(int v)
 *  int outdegree(int v)
